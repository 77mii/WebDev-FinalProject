<x-layout1>
    <x-slot:title>Add Your Student Project</x-slot:title>

    <div class="mx-auto p-0 rounded">
        <!-- Add Student Project Form -->
        <form method="POST" action="{{ route('student.project.store') }}">
            @csrf
            ...
            <div class="mb-6">
                ...
                <!-- Students -->
                <label class="block text-gray-700 font-semibold mt-4 mb-2">Select Students</label>
                    ...
                    @foreach ($students as $student)
                        <div class="flex items-center space-x-2">
                            <input 
                                ...
                                value="{{ $student->id }}"
                                class="mr-2"
                                {{ $student->id == $loggedInStudent->id ? 'checked disabled' : '' }}
                            >
                            <label for="student-{{ $student->id }}">
                                {{ $student->studentname }} ({{ $student->nim }})
                            </label>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="flex space-x-4">
                ...
                <a href="{{ route('gueststudentprojects.show', $project->id) }}" 
